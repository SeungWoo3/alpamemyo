...
| INT4 양자화 + 순차 오프로딩 **(X — 배제: 양자화 - 모델 정확도 저하)** | ~6.1 GB | **매우 높음 (권장)** |
| 모듈별 순차 로딩 (BF16) | ~18.4 GB | 불가능 |

**결론**: `device_map` 및 accelerate 기반 자동 오프로딩은 Alpamayo의 커스텀 토큰 처리와 비호환. INT4 양자화 (X — 배제) + 커스텀 레이어별 오프로딩이 실현 가능한 경로.
...
| **INT4 + 모듈 순차 오프로딩** **(X — 배제: 양자화)** | **~5.5-6 GB** | **~300-400s** | **가장 현실적** |
| INT4 + 전체 GPU 탑재 **(X — 배제: 양자화)** | ~5.5 GB | ~300-350s | 최적 |

**결론**: 모듈 순차 오프로딩은 이론적으로 가능하나, Autoregressive 디코딩에서 매 토큰마다 36개 레이어 순회로 전송 오버헤드가 극대화됨. INT4 양자화 (X — 배제)와의 조합이 필수.
...
| **INT4 + Pinned** **(X — 배제: 양자화)** | **~80-120s** | **~5.5 GB** | **가장 현실적** |
...
| **4. 창의적 접근** | **하이브리드 양자화 (X — 배제) + 스텝/해상도 최적화 (해상도: X — 배제)** | **~6.0-7.5 GB** | **~3.2-4.0s** | **보통** | **매우 높음** |

### 8.2 최적 조합 전략 제안

**권장 조합: INT4 양자화 (X — 배제: 모델 정확도 저하) + 디퓨전 스텝 축소 + 동적 해상도 조절 (X — 배제: 모델 정확도 저하)**
...
**Phase 1: 즉시 실행 (1-2일)**
1. 하이브리드 양자화 적용 (Attn-INT8 + FFN-INT4) **(X — 배제: 모델 정확도 저하 방식)**
2. 디퓨전 스텝 10->5 변경 및 minADE 비교
3. min_pixels 축소 (163840->81920) 테스트 **(X — 배제: 모델 정확도 저하 방식)**

**Phase 2: 단기 최적화 (1주)**
4. KV Cache INT8 양자화 적용 **(X — 배제: 양자화 기반 정확도 저하 방식)**
...
| 작업 | 예상 소요 | 예상 효과 |
|------|----------|----------|
| 하이브리드 양자화 (Attn-INT8 + FFN-INT4) **(X — 배제)** | 0.5일 | VRAM ~7 GB, 정확도 개선 |
| 디퓨전 스텝 축소 (10->5) + 품질 검증 | 0.5일 | 디퓨전 단계 2x 가속 |
| min_pixels 최적화 + 품질 검증 **(X — 배제)** | 0.5일 | 동적 메모리 25% 절감 |
| KV Cache INT8 양자화 **(X — 배제)** | 0.5일 | ~270 MB 추가 절감 |
...
| 레이어 가지치기 + 지식 증류 **(X — 배제)** | 4주 | VLM 17-50% 축소 |
