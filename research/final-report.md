...
|------|-------------|-------------|------|--------|
| **스왑 없음 (이론적 최적)** | **~4.91s** | **0s (0%)** | **≥21.52 GB 필요** | **24GB+ GPU 필요** |
...
**이론적 최적값 근거**: 4-bit 양자화 모델(8.87 GB)은 12GB VRAM 이내에서 스왑 없이 동작하며 추론 4.91초를 달성. 이 중 VLM 추론이 4.51초, 나머지가 모델 구동 오버헤드. FP16 모델도 스왑만 제거하면 **동일한 ~4.91초 수준**의 추론이 가능함을 시사. 즉, 현재 273.79초 중 **약 269초(98%)가 순수 스왑 오버헤드**이다.

**결론**: Pinned memory 사용이 필수 (특히 D2H). 2-8MB 청크 전송으로 대역폭 1.6x 향상 가능. 스왑 없는 이론적 최적(~4.91초)과 현재(273.79초)의 격차를 얼마나 좁히느냐가 연구의 핵심 성과 지표. 네이티브 Linux 환경 전환 시 WSL2 대비 40% 추가 개선 가능.
