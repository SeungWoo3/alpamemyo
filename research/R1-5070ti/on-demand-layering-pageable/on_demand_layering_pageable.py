"""
On-Demand Layering (Pageable Memory, No Prefetch) — RTX 5070 Ti

on-demand-layering과 동일하되 pin_memory() 미적용.
pageable vs pinned H2D 전송 속도 비교 목적.
"""
import sys
import os
import time
...
RESULTS_DIR = "/home/avees/workspace/research/R1-5070ti/on-demand-layering-pageable"
...
class OnDemandLayeringPageableHook:
    """
    On-Demand Layering 훅 (Pageable Memory, No Prefetch).
    - CPU 파라미터를 pageable memory에 보관 (pin_memory 없음)
    - pre_forward: pageable CPU → GPU (H2D)
    - post_forward: param.data를 CPU 원본으로 교체 (GPU free)
    """
    ...
    def save_cpu_refs(self, layers: nn.ModuleList, layer_indices: list):
        """CPU 레이어 파라미터 참조 저장 (pin 없음)"""
            ...
            for name, param in layer.named_parameters():
                if param.device.type == "cpu":
                    ...
                    if not data.is_contiguous():
                        ...
                        param.data = data
                    self.cpu_params[i][name] = data
                    total_params += 1
                    total_bytes += data.nelement() * data.element_size()

        t1 = time.perf_counter()
        self.setup_time = t1 - t0
        total_gb = total_bytes / 1024**3
        saved_count = len(self.cpu_params)
        print(f"    Saved {saved_count} layers ({total_params} params, {total_gb:.2f} GB) [pageable]")
        print(f"    Setup time: {self.setup_time:.2f}s")
        return saved_count
    ...
    def get_stats(self):
        ...
        return {
            ...
            "setup_time_s": round(self.setup_time, 3),
        }
...
def run_experiment():
    ...
    config_name = "on_demand_layering_pageable_no_prefetch"
    print(f"\n{'='*70}")
    ...
    t3 = time.time()
    hook = OnDemandLayeringPageableHook()
    ...
    try:
        ...
        # 하위 VLM 레이어 — CPU pageable (pin 없음)
        cpu_layer_indices = list(range(NUM_OFFLOAD))
        print(f"  Saving CPU refs for VLM layers 0-{NUM_OFFLOAD-1} [PAGEABLE]...")
        saved_count = hook.save_cpu_refs(vlm_layers, cpu_layer_indices)

        result["setup_time"] = round(time.time() - t3, 2)
        result["vlm_layers_offloaded"] = saved_count
        ...
        print(f"  Hooks registered: {saved_count} layers")
    ...
    # Phase 5: 추론
    print("\n[Phase 5] Running inference (pageable H2D + free, no prefetch)...")
    print(f"  GPU Info: {get_gpu_info()}")
    torch.cuda.reset_peak_memory_stats()
    ...
    print(f"\n{'='*70}")
    print("RESULTS (On-Demand Layering — PAGEABLE)")
    print(f"{'='*70}")
...
if __name__ == "__main__":
    print("=" * 70)
    print("Alpamayo-R1-10B — On-Demand Layering (PAGEABLE, 5070 Ti)")
    print("No pin_memory, No Prefetch")
    print("=" * 70)
    run_experiment()
