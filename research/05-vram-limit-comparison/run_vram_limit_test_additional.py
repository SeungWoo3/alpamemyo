"""
4-bit Alpamayo VRAM 제한별 성능 비교 실험 (추가 조건)
...
def main():
    print("="*70)
    print("4-bit Alpamayo VRAM Limit Comparison — Additional Experiment")
    print("="*70)
    ...
    for limit in VRAM_LIMITS:
        ...
        # 중간 결과 저장
        with open(os.path.join(RESULTS_DIR, "results_additional.json"), "w") as f:
            json.dump(all_results, f, indent=2, ensure_ascii=False)

    # 최종 결과 출력
    print(f"\n{'='*70}")
    print("FINAL RESULTS (Additional)")
    print(f"{'='*70}")
    ...
    # JSON 저장
    results_path = os.path.join(RESULTS_DIR, "results_additional.json")
